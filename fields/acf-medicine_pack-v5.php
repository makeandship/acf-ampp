...
if( !class_exists('acf_field_medicine_pack') ) :


class acf_field_medicine_pack extends acf_field {
	...
	*/
	...
	function __construct( $settings ) {
		$this->name = 'medicine_pack';
		$this->label = __('Medicine Pack', 'acf-medicine_pack');
		...
		*/
		...
		$this->l10n = array(
			'error'	=> __('Error! Please enter a higher value', 'acf-medicine_pack'),
		);
	...
	*/
	...
	function input_admin_enqueue_scripts() {
		...
		wp_enqueue_script('acf-input-medicine_pack');
		
		
		// register & include CSS
		wp_register_style( 'acf-input-medicine_pack', "{$url}assets/css/input.css", array('acf-input'), $version );
		wp_enqueue_style('acf-input-medicine_pack');
	...
	*/	
	...
	function validate_value( $valid, $value, $field, $input ){
		...
		if( $value < $field['custom_minimum_setting'] )
		{
			$valid = __('The value is too little!','acf-medicine_pack');
		}
...
// initialize
new acf_field_medicine_pack( $this->settings );
